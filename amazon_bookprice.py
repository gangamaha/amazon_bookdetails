...
# get user input and import packages according to Python version
...
The product details and the prices from the web search is written in file keyword_details.txt
The URL's are stored in keyword_links.txt
"""
# if keyword is empty gracefully exit
if keyword is '':
...
for link in overall_search_results:
    if link.startswith("https"):  # remove any incorrect URL's
        remove_dummy_links.append(link)

remove_dummy_links = list(set(remove_dummy_links))  # remove any possible duplicate URL's

links_file = (keyword+"_links.txt").replace("+", "_")
print("Writing the search result links in "+links_file)
with open(links_file, "a") as out:
    # since file is opened in append mode, same search will be updated in same output file with timestamp
...
output.close()
# limit_search = 20  # if we want to limit our price search to only 20 URL's
